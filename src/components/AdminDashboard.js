  ...
  if (!professor) {
    return (
      <div className="p-6">
        ...
        <p className="text-black">Please ensure the user exists in mock data.</p>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6">
      <div className="rounded-2xl bg-linear-to-r from-orange-500 to-amber-600 text-white p-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3 bg-white/10 backdrop-blur px-4 py-2 rounded-lg">
          ...
          <span className="px-2 py-1 text-xs bg-white text-orange-700 rounded">{professor.role}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl shadow p-5 border border-gray-100">
          <div>
            <p className="text-black text-sm">Quick tip</p>
            <p className="text-sm mt-1 text-black">Click a course to manage its assignments.</p>
          </div>
        </div>
      </div>
